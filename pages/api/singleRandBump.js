...
export async function handler(req, res) {
  ...
  const q = query(collection(db, 'users'), where('communityIds', 'array-contains', communityId))
  const users = await getDocs(q)

  users.forEach((user) => {
    const userData = user.data()
    // if they opted in and if it's been longer than 2 weeks from the last sent
      ...
      body: `Hi, Loop Bot here! ${person1.firstName} meet ${person2.firstName}! We're connecting you because${
              ...
              (ask) =>
                `${
                  ask == 'investors'
                    ...
                    : ask == 'cofounders'
                    ? `${
                        common.asksUser1[common.asksUser1.length - 1] === 'cofounders' && common.asksUser1.length !== 1
              ...
              (ask) =>
                `${
                  ask == 'investors'
                    ...
                    : ask == 'cofounders'
                    ? `${
                        common.asksUser2[common.asksUser2.length - 1] === 'cofounders' && common.asksUser2.length !== 1
                          ? ' and '
                          : ' '
                      }finding a co-founder`
                    : ask == 'refer'
        ...
        common.interests !== undefined
          ? `You also have a shared interest in${common.interests.map(
              (interest) =>
                `${
                  interest === common.interests[common.interests.length - 1] && common.interests.length !== 1 && ' and'
                } ${interest}`
            )}.`
          : ''
